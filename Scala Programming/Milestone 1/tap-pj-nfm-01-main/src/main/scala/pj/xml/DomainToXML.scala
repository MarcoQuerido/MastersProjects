...
object DomainToXML:
  ...
  def generateOutputXML(output: Either[DomainError, List[TaskSchedule]], humanIdToName: Map[String, String]): Result[Elem] =
    output match
      case Left(error: ImpossibleSchedule) => Right(generateErrorXML2(error))
      case Left(error)                    => Right(generateErrorXML(error))
      case Right(schedule)                => Right(scheduleListToXML(schedule, humanIdToName))
  ...
  private def scheduleListToXML(schedules: List[TaskSchedule], humanIdToName: Map[String, String]): Elem =
    <Schedule xmlns="http://www.dei.isep.ipp.pt/tap-2025"
      ...
      { schedules
      .sortBy(ts => (ts.orderId, ts.startTime))
  ...
  private def scheduleToTaskScheduleXML(ts: TaskSchedule, humanIdToName: Map[String, String]): Elem =
    <TaskSchedule order={ts.orderId}
                  productNumber={ts.productNumber.toString}
                  task={ts.taskId}
                  start={ts.startTime.toString}
                  end={ts.endTime.toString}>
      <PhysicalResources>
        { ts.allocatedPhysicalResources.map(id => <Physical id={id}/>) }
      </PhysicalResources>
      <HumanResources>
        { ts.allocatedHumanResources.map(id => <Human name={humanIdToName.getOrElse(id, id)}/>) }
      </HumanResources>
    </TaskSchedule>
