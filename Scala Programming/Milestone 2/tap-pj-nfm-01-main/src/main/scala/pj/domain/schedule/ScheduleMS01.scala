    ...
    input.orders match
      ...
      case nonEmptyOrders =>
        val productMap: Map[ProductId, Product] =
          input.products.map(p => p.productId -> p).toMap // A fast lookup table to retrieve products by their ID

        // Do all orders refer to valid products?
        nonEmptyOrders.find(order => !productMap.contains(order.productId)) match
          case Some(missingOrder) => // if an order points to a non existant product
            Left(DomainError.ProductDoesNotExist(missingOrder.productId))

          case None => // If a product is missing for an order, return an error
            nonEmptyOrders.sortBy(_.orderId.to).foldLeft( // Sort orders by their ID (to ensure deterministic scheduling)
              ...
              Right((0, List.empty[TaskSchedule])): Result[(Int, List[TaskSchedule])]
                  ...
                  // for each order
              case (Right((currentTime, acc)), order) => // Gets the product for this order
