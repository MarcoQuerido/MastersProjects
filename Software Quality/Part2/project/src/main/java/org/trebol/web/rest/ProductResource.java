...
import org.trebol.repository.ProductRepository;
import org.trebol.service.ProductService;
...
/**
 * REST controller for managing {@link org.trebol.domain.Product}.
 */
@RestController
@RequestMapping("/api")
public class ProductResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/products/uploadfields")
    public ResponseEntity<ProductDTO> createProduct(@Parameter String name, @Parameter String barcode,
                                                    @Parameter String description, @Parameter Integer price,
                                                    @Parameter Integer stockCurrent, @Parameter Integer stockCritical) throws URISyntaxException {
        if ((name == null || name.isEmpty()) || (barcode == null || barcode.isEmpty()) ||
            (price == null || price <= 0) || (stockCurrent == null || stockCurrent <= 0) ||
            (stockCritical == null || stockCritical <= 0)) {
            System.out.println("Name: "+name);
            System.out.println("Barcode: "+barcode);
            System.out.println("Price: "+price);
            System.out.println("Stock Current: "+stockCurrent);
            System.out.println("Stock Critical: "+stockCritical);

            throw new BadRequestAlertException("Invalid fields", ENTITY_NAME, "null or empty");
        }

        log.debug("REST request to save Product : {}");
        ProductDTO productDTO = new ProductDTO();
        log.debug("Create empty Product");
        productDTO.setName(name);
        log.debug("Set Product name");
        productDTO.setBarcode(barcode);
        log.debug("Set Product barcode");
        productDTO.setDescription(description);
        log.debug("Set Product description");
        productDTO.setPrice(price);
        log.debug("Set Product price");
        productDTO.setStockCurrent(stockCurrent);
        log.debug("Set Product stockCurrent");
        productDTO.setStockCritical(stockCritical);
        log.debug("Set Product stockCritical");
        ProductDTO result = productService.save(productDTO);
        return ResponseEntity
            .created(new URI("/api/products/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, result.getId().toString()))
