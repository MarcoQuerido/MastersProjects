    ...
    public void run(String... args) throws IOException, InterruptedException {
        ...
        rpcResponse.reviews.forEach(reviewDTO -> {
                    try {
                        ...
                        reviewService.addReview(reviewDTO.getId(), reviewDTO.getReviewStatus(), null);
                    } catch (Exception e) {
                        logger.error(e.getMessage());
                    }
                }
        ...
        rpcResponse.votes.forEach(voteDTO -> {
                    try {
                        logger.info(String.format("Adding the following vote: %s", voteDTO.toString()));
                        voteService.addVoteToReview(voteDTO.getReviewId(), voteDTO);
                    } catch (Exception e) {
                        logger.error(e.getMessage());
                    }
                }
